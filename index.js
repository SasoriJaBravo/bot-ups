...
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  try {
    ...
    if (!interactionManager.isSafe(interaction)) {
      console.error("❌ Interação não é segura para exibir modal");
      return await respostaRapida(interaction, {
        content: "❌ Tempo limite excedido. Tente novamente.",
        ephemeral: true
      });
    }
    ...
    if (error.code === 10062) {
      console.error("❌ Erro 10062: Não foi possível exibir modal - interação expirada");
      // Não tentar resposta de emergência para erro 10062
    } else {
      console.error("❌ Erro ao exibir modal:", error.message);
      // Tentar resposta de emergência para outros erros
      try {
        if (interactionManager.isSafe(interaction)) {
          await respostaRapida(interaction, {
...
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  if (acao === "reprovar") {
    ...
    try {
      ...
      if (error.code === 10062) {
        console.error("❌ Erro 10062: Não foi possível exibir modal de reprovação - interação expirada");
        // Não tentar resposta de emergência para erro 10062
      } else {
...
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  await operacaoSegura(
    ...
    },
    {
      ...
      mensagemSucesso: `✅ **Reprovação Registrada com Sucesso**\n\n❌ **Candidato:** ${membro.displayName}\n📝 **Motivo:** ${motivo}\n📊 **Resultados:** Registrado no canal de resultados\n📨 **Notificação:** DM enviada ao candidato`,
      ephemeral: true,
      defer: true
    }
  );
...
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  await operacaoSegura(
    ...
    },
    {
      ...
      mensagemSucesso: `🎉 **Aprovação Concluída com Sucesso!**\n\n✅ **Candidato:** ${membro.displayName}\n🏷️ **Cargo Concedido:** ${role.name}\n📝 **Nickname Atualizado:** ${membro.nickname || "Nome padrão"}\n🏆 **Placar:** Atualizado automaticamente\n📊 **Resultados:** Registrado no canal de resultados`,
      ephemeral: true,
      defer: true
    }
  );
